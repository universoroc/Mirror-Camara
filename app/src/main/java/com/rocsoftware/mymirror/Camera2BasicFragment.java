...
public class Camera2BasicFragment
   ...
   // private ImageReader mImageReader2; // for reflexion
    ...
    /** the last photo in bitmap format
     *
     */
    ...
    private static int openCameraCount=0;
    private static int closeCameraCount=0;
    ...
    private static final String PREF_SOUND_EFFECT = "preference_sound_effect";
    ...
    private static final String PREF_SKBZOOM = "PREF_SKBZOOM";

    private Boolean mPrefSoundEffect=true;
    private Boolean mSecondActivity=false; // Flag

    private OnCameraClickedCallBackListener mCallback;
    private OnCameraReadyCallBackListener mCallbackReadyCamera;



    private List<String> color_effects = null;
    private int mColorEffect = CameraMetadata.CONTROL_EFFECT_MODE_OFF;
    private int[] mControlAvailableEffects;
    private int mIndexControlAvailableEffects=0;

    CameraCharacteristics characteristics;

    public int maxZoomLevel = 0; // max idx in zoom_ratios;
    private List<Integer> zoom_ratios = null;
    private Boolean is_zoom_supported=false;
    private int mCurrentZoomLevel=0; // current idx in zoom ratios
    private Rect scalar_crop_region = null;


    private Range mExposureTimeRange;

    long debug_time = 0;

    /* start this fragment lifecycle and interface lifecycle  */

    /*@ROC:: Constructor must be of this type because this is a fragment and
    **       thi is a restriction of java...
    **/
    ...
    public interface OnCameraClickedCallBackListener {
    ...
    // Container Activity must implement this interface
    public interface OnCameraReadyCallBackListener {
        public void onCameraReadyCallBack(
                                          int mColorEffect,
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mCallback = (OnCameraClickedCallBackListener) activity;
            mCallbackReadyCamera = (OnCameraReadyCallBackListener) activity;
        } catch (ClassCastException e) {
            throw new ClassCastException(activity.toString()
                    + " must implement OnCameraClickedListener");
        }
    }
...
// Begin Listeners and Other Methods.
    ...
    private final TextureView.SurfaceTextureListener mSurfaceTextureListener
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture texture, int width, int height) {
            ...
            mCallbackReadyCamera.onCameraReadyCallBack( mColorEffect, mCurrentZoomLevel );
        }
...
*/
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            new MyAsyncTask().execute(reader.acquireNextImage());

        }

    };

    private class MyAsyncTask extends AsyncTask<Image, Integer, String > {
        @Override
...
// Moved to a background thread.
            ...
            ImageSaverAsyncTask p = new ImageSaverAsyncTask(parameter[0], mFile);
            p.runSaverImage();
...
// Report results via UI update, Dialog, or notifications
          //  asyncTextView.setText(result);
            ...
            // Talk to main activity...
            mCallback.onCameraClickedCallBack(mCameraDevice,
                                              mPreviewRequest,
                                              mCaptureSession,
                                              mFile,
                                              mImageBitmap);
    ...
    private void findCameraCharacteristics() {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
               ...
               // MyLog.d(TAG,"ExposureTime Range: "+mExposureTimeRange.toString());
                checkSupportZoom();
    ...
    public Integer getAeCurrentCompensation() {

                return(mPreviewRequest.get(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION));



    }

    public int getColorEffect() { return this.mColorEffect; }

    public void setNextColorEffect() {
        try {
            int lcae = mControlAvailableEffects.length;
            mIndexControlAvailableEffects = (++mIndexControlAvailableEffects) % lcae;
            Integer newColorEffect = mControlAvailableEffects[mIndexControlAvailableEffects];
            setColorEffect(newColorEffect);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private  void setColorEffect(Integer newColorEffect) {
        try {
            if (newColorEffect != null && newColorEffect != mColorEffect) {
                mColorEffect = newColorEffect;
    ...
    private boolean setColorEffectOrg(CaptureRequest.Builder builder) {
			/*if( builder.get(CaptureRequest.CONTROL_EFFECT_MODE) == null && color_effect == CameraMetadata.CONTROL_EFFECT_MODE_OFF ) {
				// can leave off
			}
			else*/ if( builder.get(CaptureRequest.CONTROL_EFFECT_MODE) == null || builder.get(CaptureRequest.CONTROL_EFFECT_MODE) != mColorEffect ) {
            MyLog.d(TAG, "setting color effect: " + mColorEffect);
            builder.set(CaptureRequest.CONTROL_EFFECT_MODE, mColorEffect);
            return true;
        }
        return false;
    }
    ...
    private void checkSupportZoom() {
        float max_zoom = characteristics.get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
        this.is_zoom_supported = max_zoom > 0.0f;

            MyLog.d(TAG, "max_zoom: " + max_zoom);
        if( this.is_zoom_supported ) {
            ...
            final double scale_factor = Math.pow(max_zoom, 1.0/(double)n_steps);
                ...
                MyLog.d(TAG, "n_steps: " + n_steps);
                MyLog.d(TAG, "scale_factor: " + scale_factor);

            this.zoom_ratios = new ArrayList<Integer>();
            this.zoom_ratios.add(100);
            double zoom = 1.0;
            for(int i=0;i<n_steps-1;i++) {
                zoom *= scale_factor;
                this.zoom_ratios.add((int)(zoom*100));
            }
            this.zoom_ratios.add((int)(max_zoom*100));
            this.maxZoomLevel = this.zoom_ratios.size()-1;
            MyLog.d(TAG,"ratios size: "+Integer.toString(this.maxZoomLevel));
    ...
    public Boolean isZoomSupported() { return this.is_zoom_supported; }

    public int getCurrentZoomLevel() { return this.mCurrentZoomLevel; }
    ...
    public void setZoom(int value) {
        try {
            ...
            if (zoom_ratios == null) {
                ...
                MyLog.d(TAG, "zoom not supported");
                return;
            }
            if (value < 0 || value > zoom_ratios.size()) {

                    MyLog.e(TAG, "invalid zoom value: " + value);
                throw new RuntimeException(); // throw as RuntimeException, as this is a programming error
            }
            if (! mTextureView.isAvailable()) {
                MyLog.e(TAG, "setZoom: Preview not available...");
                return;
            }
            ...
            bottom += hheight;
                ...
                MyLog.d(TAG, "zoom: " + zoom);
                MyLog.d(TAG, "hwidth: " + hwidth);
                MyLog.d(TAG, "hheight: " + hheight);
                MyLog.d(TAG, "sensor_rect left: " + sensor_rect.left);
                MyLog.d(TAG, "sensor_rect top: " + sensor_rect.top);
                MyLog.d(TAG, "sensor_rect right: " + sensor_rect.right);
                MyLog.d(TAG, "sensor_rect bottom: " + sensor_rect.bottom);
                MyLog.d(TAG, "left: " + left);
                MyLog.d(TAG, "top: " + top);
                MyLog.d(TAG, "right: " + right);
                MyLog.d(TAG, "bottom: " + bottom);

            this.scalar_crop_region = new Rect(left, top, right, bottom);
           // camera_settings.setCropRegion(previewBuilder);
            this.mCurrentZoomLevel= value;
            if( scalar_crop_region != null ) {
                mPreviewRequestBuilder.set(CaptureRequest.SCALER_CROP_REGION, scalar_crop_region);
                ...
                MyLog.e(TAG, "failed to set zoom");
                MyLog.e(TAG, "message: " + e.getMessage());

            e.printStackTrace();
        }
    }

    private void setZoomCropRegion(int value) {
        try {
            ...
            MyLog.e(TAG, "failed to prepare  zoom crop region");
            MyLog.e(TAG, "message: " + e.getMessage());
    ...
    private void openCamera(int width, int height) {
        ...
        mSecondActivity = sharedPreferences.getBoolean(PREF_SECOND_ACTIVITY_RUN, false);
        if (  mSecondActivity ) {
            // running after second activity and the session variable lost his value.
            // we obtain it from preferences.
            // This situation do not succede at direct run from user.
            // zoom level and color effect are remenbered using preferences after using settings or about dialog
            // the flag mSecondActivity will be change to false.
            mCurrentZoomLevel = sharedPreferences.getInt(PREF_SKBZOOM, 0);
            setZoomCropRegion( mCurrentZoomLevel );
            mColorEffect = sharedPreferences.getInt(PREF_COLOR_EFFECT, CameraMetadata.CONTROL_EFFECT_MODE_OFF);
            setFlagUnloadSecondActivity( sharedPreferences );
    ...
    private void setFlagUnloadSecondActivity( SharedPreferences sharedPreferences ) {
...
*/
                ...
                mControlAvailableEffects = characteristics.get(CameraCharacteristics.CONTROL_AVAILABLE_EFFECTS);

                MyLog.d(TAG,"mControlEffect: longitud: "+mControlAvailableEffects.length);
...
*/
    ...
    private Size getOptimalPreviewSize(Activity activity,
                                      CameraCharacteristics characteristics,
    ...
    private Size getClosestSize(Size[] sizes, double targetRatio) {
     ...
     */
    private void configureTransformv2(int viewWidth, int viewHeight) {
        Activity activity = getActivity();
        Point p = new Point();
        Matrix matrix;
        RectF viewRect;
        ...
        /** mTextureView.Surfacesize <= mPreviewSize; */

        MyLog.d(TAG,"configureTransform:  width: "+ viewWidth + " height: "+viewHeight);
  ...
  //      if (Surface.ROTATION_90 == rotation || Surface.ROTATION_270 == rotation) {
          ...
          //  matrix.postRotate(90 * (rotation - 2), centerX, centerY);
            ...
            MyLog.d(TAG,"configureTransform:  Rotacion 180");
            activity.getWindowManager().getDefaultDisplay().getSize(p);
            MyLog.d(TAG, "configureTransform: point: "+p.toString());
            viewWidth = p.x; viewHeight= p.y;
            viewRect = new RectF(0, 0, viewWidth, viewHeight);
            bufferRect = new RectF(0, 0, mPreviewSize.getHeight(), mPreviewSize.getWidth());
            centerX = viewRect.centerX();
            centerY = viewRect.centerY();
    ...
    private void createCameraPreviewSession() {
        try {
            ...
            mPreviewRequestBuilder.set(CaptureRequest.SENSOR_SENSITIVITY, CameraMetadata.SENSOR_REFERENCE_ILLUMINANT1_DAYLIGHT);

            mPreviewRequestBuilder.set(CaptureRequest.SHADING_MODE, CameraMetadata.SHADING_MODE_HIGH_QUALITY);

            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_ANTIBANDING_MODE, CameraMetadata.CONTROL_AE_ANTIBANDING_MODE_AUTO);


            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_SCENE_MODE, CameraMetadata.CONTROL_SCENE_MODE_STEADYPHOTO);

            mPreviewRequestBuilder.set(CaptureRequest.JPEG_QUALITY, (byte)90);

            mPreviewRequestBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE,0.0F);

            mPreviewRequestBuilder.set(CaptureRequest.LENS_FOCAL_LENGTH, 2.2F);
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(surface, mImageReader.getSurface()),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(
                            ...
                            showToast(getActivity().getResources().getString(R.string.sentence_configure_failed));
                        }
                    }, null
            );
        } catch (CameraAccessException e) {
    ...
    public void takePicture() {
        ...
        lockFocus();
        Log.d(TAG, "total time takepicture lockFocus: " + (System.currentTimeMillis() - debug_time));
        galleryAddPic(mFile);
        Log.d(TAG, "total time takepicture galleryAddPic: " + (System.currentTimeMillis() - debug_time));
    ...
    private void initCameraSound() {
        ...
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(activity.getBaseContext());
        mPrefSoundEffect = sharedPreferences.getBoolean(SettingsActivity.KEY_PREF_SOUND_EFF,true);
    ...
    private void playCameraSound() {
     ...
     //   Activity activity = getActivity();
            ...
            if (hasCameraSound && mPrefSoundEffect) {
                MyLog.d(TAG, " <LN: " + Thread.currentThread().getStackTrace()[2].getLineNumber() + " > " + "Voy a play sound");
                soundPool.play(soundId, 1f, 1f, 1, 0, 1f); //.play nos pide el int de nuestra canción, un //int float de volumen tanto derecho como izquierdo que van de 0.0=0% a 1.0=100%, el int de loop //donde -1 es reproducción en loop, 0 reproduce una vez y 1 repite sólo una vez, y por último tenemos //un int de Rate que va de 0.5 a 2 y modifica la velocidad de pitch donde 1.0 seria la velicidad normal
            }
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               @NonNull CaptureRequest request,
                                               @NonNull TotalCaptureResult result) {
                    showToast(activity.getResources().getString(R.string.saved) +": " + mFile);
    ...
    private  static class ImageSaverAsyncTask  {
    ...
    private File createImageFile() {
        ...
        try {
            ...
            MyLog.d(TAG,"File Absolute: "+mFile.getAbsolutePath());
    ...
    private void galleryAddPic(File image ) {
        // Broadcast the image to galley and others intents.
        ...
        Activity activity = getActivity();
        Uri contentUri = getImageContentUri(activity, image);
       ...
       // Activity activity = getActivity();
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(activity.getBaseContext());
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putString(PREF_FULLPATHLASTPHOTO, image.getAbsolutePath());
        editor.putString(PREF_URILASTPHOTO, contentUri.toString());
        editor.apply();
        editor.commit();
        MyLog.d(TAG,"Uri galleryadd: "+contentUri.toString());
    }
    ...
    private static Bitmap reflexionImage(byte[] bytes_imagen , int w, int h) {
        ...
        Bitmap reflexionBitmap = Bitmap.createBitmap(BitmapOrg, 0, 0, width, height, matrix, true);
